...
async def give_admin_root(message: Message):
    managers = db.get_managers_from_db()
...
async def show_time_until_notification(message: Message):
    ...
    start_time = belarus_tz.localize(
        datetime.now().replace(hour=18, minute=50, second=0, microsecond=0)
    )
    end_time = belarus_tz.localize(
    ...
    if not start_time.time() <= current_time.time() <= end_time.time():
        ...
        if time_sleep.total_seconds() > 0:
            ...
            await message.answer(
                f"Бот спит! Проснется через {hours} часов, {minutes} минут, {seconds} секунд"
            )
        else:
            ...
            minutes, seconds = divmod(remainder, 60)
            await message.answer(
                f"Бот спит! Проснется завтра через {hours} часов, {minutes} минут, {seconds} секунд"
            )
    else:
