...
class DataBase:
    ...
    def add_report_to_db(self, report: Report):
        try:
            self._cursor.execute(
                """INSERT INTO reports (date, name, tg_id, orders, invoices, paid_invoices, margine,
                ...
                (
                    ...
                    report.margin,
                    report.revenue,
                    round(report.conversion, 2),
                    round(report.conversion_paid, 2),
                    round(report.markup_percentage, 2),
                ),
            )
    ...
    def change_manager_role(self, id, current_role: str) -> str:
        if current_role == "SuperManager":
            self._cursor.execute(
                f"""UPDATE managers SET role = 'Manager' WHERE id == '{id}'"""
            )
            self._connection.commit()
            return "Manager"

        self._cursor.execute(
            f"""UPDATE managers SET role = 'SuperManager' WHERE id == '{id}'"""
        )
        self._connection.commit()
        return "SuperManager"
